...
__global__ void tcfSpmmKernel(
    ...
    const int group = lane >> 2;        // 读哪些列
    const int threadInGroup = lane & 3; // 读X的第几行
    ...
    for(OffsetType tile = tileBegin; tile < tileEnd; tile++){
        ...
        if(thread < TILE_ROWS * COL_WINDOW_WIDTH) {
            const BitmapType currentBit = BitmapType{1} << thread; // 生成只有第thread位置为1的bitmap
            
            if(bitmap & currentBit){
                const BitmapType lowerBits = bitmap & (currentBit - 1);
                const OffsetType valueIndex = valueBegin + static_cast<OffsetType>(__popcll(static_cast<unsigned long long>(lowerBits))); // 当前非0元素是第几个元素
...
void launchTcfSpMM(
    ...
    tcfSpmmKernel<<<grid, block, 0, stream>>>(
        colWindowOffset,
        tileOffset,
        tileRowIndices,
        tileLocalBit,
