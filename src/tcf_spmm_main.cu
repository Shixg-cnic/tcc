...
int main(int argc, char** argv) {
    ...
    const std::string outputPath =
        argc > 2
            ? argv[2]
            : "/cnic/work/shixg/GNN/tc_cc_kernel/TccSpmm/test/data/arxiv_tcf_output.bin";
    ...
    for(int repeat = 0; repeat < warmupCount; ++repeat) {
        tcc::launchTcfSpMM(deviceColWindowOffset, deviceTileOffset, deviceTileRowIndices, deviceTileLocalBit, deviceValues, deviceX, deviceY, rows, cols, numColWindows, featureDim);
    }
