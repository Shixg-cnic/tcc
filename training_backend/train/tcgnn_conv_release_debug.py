    ...
    def forward(
        ...
        if device_id == 0: 
            ...
            if latest_folder:
                ...
                np.savetxt(AX_file_path, X_out.cpu().numpy(), fmt="%10f", delimiter=",")  # 保存为 CSV
                print(f"AX result 已保存至: {AX_file_path}")

                # AXW result
                AXW_file_name = "AXW_1_layer.csv" if first_layer else "AXW_2_layer.csv"
                AXW_file_path = os.path.join(save_path, AXW_file_name)

                np.savetxt(AXW_file_path, X_gat.cpu().numpy(), fmt="%10f", delimiter=",")  # 保存为 CSV
                print(f"AXW result 已保存至: {AXW_file_path}")
                ...
                np.savetxt(row_file_path, row_indices.cpu().numpy(), fmt="%d", delimiter=",")  # 保存为 CSV
                np.savetxt(col_file_path, col_indices.cpu().numpy(), fmt="%d", delimiter=",")  # 保存为 CSV

                print(f"coo-row 已保存至: {row_file_path}")
                print(f"coo-col 已保存至: {col_file_path}")

                # combine coo
                combine_coo_file_name = "coo_1_layer.csv" if first_layer else "coo_2_layer.csv" 
                ...
                np.savetxt(weights_file_path, weights.cpu().numpy(), fmt="%10f", delimiter=",")  # 保存为 CSV

                print(f"coo-row 已保存至: {weights_file_path}")

                # second layer x（第二层的 X 并不只是第一层的 AXW 输出，因为还经过了 relu 和 dropout 运算）
                if first_layer == False:
                    x_file_name = "x_2_layer.csv"
                    x_file_path = os.path.join(save_path, x_file_name)

                    np.savetxt(x_file_path, x_2d.cpu().numpy(), fmt="%10f", delimiter=",")

                    print(f"second layer x 已保存至: {x_file_path}")
            else:
...
def train_one_step(model, optimizer, metric, loss_fcn, device, feat_len, iter, device_id):
    ...
    if device_id == 0: 
        ...
        if latest_folder:
            ...
            file_path = os.path.join(save_path, file_name)
            np.savetxt(file_path, features.cpu().numpy(), fmt="%10f", delimiter=",")  # 保存为 CSV(%6.2f)
