...
class CNICSPMMFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        x, weights, weight_vertical_axis_dim, device):
...
class GCNConv(torch.nn.Module):
    ...
    def forward(
        ...
        return CNICSPMMFunction.apply(
            ...
            cpu_float_feature_len, gpu_node_capacity, feature_dim,
            ids, cpu_feature_cache_ptr, cache_search_map, gpu_feature_cache_ptr,
            x, self.weights, self.weight_vertical_axis_dim, device)
    ...
    def forward(
        ...
        x = self.relu(
            self.conv1(
                ...
                coo_data,
                cpu_float_feature_len, gpu_node_capacity, feature_dim,
                ids, cpu_feature_cache_ptr, cache_search_map, gpu_feature_cache_ptr, device, x)
        )
        x = self.dropout(x)
