...
int main(int argc, char** argv) {
    ...
    const std::string outputPath =
        argc > 2
            ? argv[2]
            : "/cnic/work/shixg/GNN/tc_cc_kernel/test/data/arxiv_tcf_hybrid_output.bin";
    ...
    std::size_t tcTiles = 0;
    std::size_t ccTiles = 0;

    for(const tcc::BitmapType bitmap : tcf.tileLocalBit) {
        const tcc::IndexType tileNnz =
            static_cast<tcc::IndexType>(
        ...
        if(tileNnz >= tcc::HYBRID_TC_THRESHOLD) {
            ++tcTiles;
        }
        else {
            ++ccTiles;
    ...
    for(int repeat = 0; repeat < warmupCount; ++repeat) {
        tcc::launchTcfSpMMHybrid(deviceColWindowOffset, deviceTileOffset, deviceTileRowIndices, deviceTileLocalBit, deviceValues, deviceX, deviceY, rows, cols, numColWindows, featureDim);
    }
